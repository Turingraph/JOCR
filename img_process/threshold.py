import cv2
import numpy as np
from ImgProcess.Utility import SetPx, GetDefaultOption, odd_area
    ...
    def Edit(self, img: np.ndarray):
        if type(self.threshold_px) == int:
            return cv2.threshold(
                img, self.threshold_px, self.max_px, self.method
            )[1]
        else:
            return cv2.threshold(
                img, 0, self.max_px, self.method + cv2.THRESH_OTSU
            )[1]
...
class AdaptiveThreshold:
    def __init__(
        ...
        self.method = GetDefaultOption(method, method_options, short_message)
        self.kernel_area = odd_area(kernel_area)
        self.constant = constant
        self.max_px = SetPx(max_px)
        self.adaptive_method = GetDefaultOption(
            adaptive_method,
