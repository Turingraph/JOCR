import cv2 
import numpy as np
from image_utility import odd_kernel_area

def mean_blur(img:np.ndarray , kernel_area:int = 15, scalar:None|int|float = None) -> :
...
def gauss_blur(img:np.ndarray, kernel_area:int = 15):
...
def bilateral_blur(img:np.ndarray, kernel_area:int = 15, effect:int = 75):
